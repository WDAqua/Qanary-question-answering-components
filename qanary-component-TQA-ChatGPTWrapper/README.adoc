...
It sends the given question to the https://platform.openai.com/docs/api-reference/completions[completions API] and store all returned choices in the graph.

As a Qanary component, it reads the question and the annotations produced by upstream components from the Qanary triplestore (the shared process memory), performs its task — answer the question directly from text using a language model — and writes the result back as standard Qanary (Web Annotation / `qa:`) annotations. See <<Input specification>> / <<Output specification>> below for the exact annotation contract.

== Characteristics

* *Task type:* Textual Question Answering (TQA).
* *Note:* This Qanary component is requesting an answer from the ChatGPT API and stores it into the Qanary triplestore.
...
* *Main component class:* `src/main/java/eu/wdaqua/qanary/component/chatgptwrapper/tqa/ChatGPTWrapper.java` (extends `eu.wdaqua.qanary.component.QanaryComponent`).
* *Tests:* 8 test class(es) under `src/test/java/` (`CacheTests.java`, `ChatGPTRestControllerTest.java`, `ChatGPTTestConfiguration.java`, `ChatGPTWrapperMockedTest.java`, `MyCompletionRequestTest.java`, `MyOpenAiApiMockedTest.java`).
...
docker build --build-arg JAR_FILE=target/qanary-component-tqa-chatgptwrapper.jar -t qanary/qanary-component-tqa-chatgptwrapper .
----
