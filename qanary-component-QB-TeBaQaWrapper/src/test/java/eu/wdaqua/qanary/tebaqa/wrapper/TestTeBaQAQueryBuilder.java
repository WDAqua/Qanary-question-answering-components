...
@WebAppConfiguration
public class TestTeBaQAQueryBuilder {
    // time span for caching, tests wait this time span during the test runs
    protected final static int MAX_TIME_SPAN_SECONDS = 5;
    private static final Logger logger = LoggerFactory.getLogger(TestTeBaQAQueryBuilder.class);
    private final String applicationName = "TestTeBaQAQueryBuilder";
    ...
    public void testIsLangSuppoerted() throws Exception {
        ...
        TeBaQAQueryBuilder teBaQAQueryBuilder = new TeBaQAQueryBuilder(threshold, langDefault, supportedLang, this.endpoint,
                this.applicationName, this.restTemplate, myCacheOfResponse);

        assertTrue(teBaQAQueryBuilder.isLangSuppoerted("en"));

        assertFalse(teBaQAQueryBuilder.isLangSuppoerted("fr"));
        assertFalse(teBaQAQueryBuilder.isLangSuppoerted("es"));
        assertFalse(teBaQAQueryBuilder.isLangSuppoerted("ne"));
        assertFalse(teBaQAQueryBuilder.isLangSuppoerted("de"));
        assertFalse(teBaQAQueryBuilder.isLangSuppoerted("se"));
    }
    ...
    @Test
    public void liveTest() throws URISyntaxException {
        float threshold = 0.5f;
        String langDefault = "en";
        ArrayList<String> supportedLang = new ArrayList<String>(Arrays.asList("en"));
