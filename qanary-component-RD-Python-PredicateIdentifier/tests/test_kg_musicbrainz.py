"""Predicate identification over a 100-resource subset of MusicBrainz.

Fixture: ``kg_fixtures/musicbrainz.json``. MusicBrainz has no live SPARQL/RDF
endpoint, so the subset is built from the MusicBrainz web-service (WS2) data and
mapped onto the community Music-Ontology / FOAF / Dublin-Core predicate URIs
(the vocabulary used by the historical LinkedBrainz RDF export). Each case gives
a natural-language question and a resource URI; the component must return the
expected predicate URI. Run ``pytest -v`` to see, per case,
