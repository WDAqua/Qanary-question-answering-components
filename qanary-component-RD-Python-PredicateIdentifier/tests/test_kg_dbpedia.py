"""Predicate identification over a 100-resource subset of DBpedia.

Fixture: ``kg_fixtures/dbpedia.json`` (built from the public DBpedia SPARQL
endpoint). Each case gives a natural-language question and a resource URI; the
component must return the expected predicate URI (chosen out of the resource's
full predicate inventory). Run ``pytest -v`` to see, per case,
