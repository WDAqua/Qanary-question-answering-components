...
* *External service:* https://api.dandelion.eu/datatxt/nex/v1/?text=
...
* *Main component class:* `src/main/java/eu/wdaqua/qanary/component/dandelion/ned/DandelionNED.java` (extends `eu.wdaqua.qanary.component.QanaryComponent`).
* *Tests:* 6 test class(es) under `src/test/java/` (`CacheTests.java`, `DandelionLiveTest.java`, `DandelionMockedTest.java`, `DandelionTestConfiguration.java`, `QanaryServiceControllerTest.java`, `SwaggerUiAvailabilityTest.java`).
...
docker build --build-arg JAR_FILE=target/qanary-component-ned-dandelion.jar -t qanary/qanary-component-ned-dandelion .
----
