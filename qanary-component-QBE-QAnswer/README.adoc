...
* *External service:* https://query.wikidata.org/bigdata/namespace/wdq/sparql
* *Note:* This Qanary component retrieves the Named Entities from the Qanary triplestore, replaces entities by the entity URIs, and fetches results for the enriched question from the QAnswer API.
...
* *Main component class:* `src/main/java/eu/wdaqua/qanary/component/qanswer/qbe/QAnswerQueryBuilderAndExecutor.java` (extends `eu.wdaqua.qanary.component.QanaryComponent`).
* *Tests:* 3 test class(es) under `src/test/java/` (`QAnswerQueryBuilderAndExecutorTest.java`, `QanaryServiceControllerTest.java`, `SwaggerUiAvailabilityTest.java`).
