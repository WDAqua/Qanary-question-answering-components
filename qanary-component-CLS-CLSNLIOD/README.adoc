...
As a Qanary component, it reads the question and the annotations produced by upstream components from the Qanary triplestore (the shared process memory), performs its task — recognise and link the ontology class referred to in the question — and writes the result back as standard Qanary (Web Annotation / `qa:`) annotations. See <<Input specification>> / <<Output specification>> below for the exact annotation contract.

== Characteristics

* *Task type:* Class Linking / Classification (CLS).
...
* *Main component class:* `src/main/java/eu/wdaqua/qanary/component/clsnliod/cls/ClsNliodCls.java` (extends `eu.wdaqua.qanary.component.QanaryComponent`).
